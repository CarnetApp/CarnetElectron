...
Search.prototype.searchInFile = function(filePath, callback){
    ...
    try {
        ...
        opener.getMainTextAndMetadata(function (txt, metadata) {
            console.log("opened")
                if(txt.indexOf(search.keyword) >= 0 || metadata.keywords.indexOf(search.keyword) >= 0 ){
                    search.result.push(new Note(stripExtensionFromName(getFilenameFromPath(filePath)), txt, filePath, metadata))
